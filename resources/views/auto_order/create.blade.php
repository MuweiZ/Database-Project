            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <!-- inline-template 代表通过内联方式引入组件 -->
                        ...
                        <form class="form-horizontal" role="form" action="{{ route('auto_order.store') }}" method="post">
                            {{ csrf_field() }}
                        <?php
                          $id = $_REQUEST['auto_policy'];
                          $price = App\Models\Insurance::where('id', App\Models\AutoPolicy::where('id', $id)->value('insurance_id'))->value('price');
                          $amount = App\Models\AutoPolicy::where('id', $id)->value('amount');

                        ?>
                        <div class="form-group row">
                            <label class="col-form-label text-md-right col-sm-3">Insurance Type</label>
                            <div class="col-sm-8" align = "left">
                                <label class="col-form-label text-md-left col-sm-4"> {{ App\Models\Insurance::where('id', App\Models\AutoPolicy::where('id', $id)->value('insurance_id'))->value('title') }} </label>
                                <input type="hidden" class="form-control" name="auto_policy_id" value={{$id}}>
                            </div>
                        </div>
                        <div class="form-group row">
